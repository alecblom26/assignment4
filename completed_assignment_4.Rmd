...
author: |
  | Alec Blom
  | a.l.n.blom@students.uu.nl
  | http://www.github.com/alecblom26
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW$treat <- ifelse(GrowthSW$revolutions == 0, "No revolutions", "More than 0 revolutions")
summary_data <- datasummary(growth + rgdp60 ~ treat*(mean + median + sd + min + max), data = GrowthSW)

summary_data_memory <- summary_data
print(summary_data)
...
resulting from the data summary, their is a difference between countries with or without a revolution. we can see this as the mean and median differ in the growth rate, this means that revolution do effect economic growth
...
The p-value is 0.06871.This is the statistical significance of the t-test. depending on the alpha value selected in the hypothesis test (usually 0.05 or 0.01), one would look whether the p-value is greater than the alpha value. In this case, that is true, therefore we would reject the null hypothesis at this level. In the case of the above, the null hypothesis, the difference of the means between the two groups is equal to zero, is rejected. We assume the difference in the two means is greater/ less than zero.
...
rgdp60 is the value of gdp per capita in 1960, converted to 1960 US Dollars. growth is the variable which is the average annual percentage growth of real GDP from 1960 to 1965. Including rgdp60 was probably done to help calculate the average growth rate per country, and used as a base value.
...
```{r}
model1 <- lm(data = GrowthSW, growth ~ treat)
model2 <- lm(data = GrowthSW, growth ~ treat + rgdp60)
model3 <- lm(data = GrowthSW, growth ~ treat + rgdp60 + tradeshare)
model4 <- lm(data = GrowthSW, growth ~ treat + rgdp60 + tradeshare + education)
```
...
I think tradeshare, as once tradeshare is introduced into the model, the value of the correlation is cloest to 0, indicating the strongest positive correlation in the model.
